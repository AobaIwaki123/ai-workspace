...
- **検証手順**:
  1. Windows Update によるホスト OS の再起動を実行
  2. 再起動後、Windows のログオン画面（PIN 入力待ち）の状態で PC に一切触れずに放置
  3. クライアント端末（Mac）のターミナルから `ssh wsl` を実行
- **検証結果 (完全合格・完全ハンズフリー実証)**:
  - **Windows 側の PIN 入力・手動ログインを一切行うことなく、Mac から `ssh wsl` だけで一発接続に成功**。
  - イーサネットの固定 IP（`192.168.11.15`）および公開鍵認証（パスワードなし）が正常に機能。
- **メカニズムと知見**:
