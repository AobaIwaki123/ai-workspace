...
# ドキュメント・Mermaid 構文の規約 (Document & Mermaid Integrity)

- **チャット返答本文における Mermaid 出力の禁止**:
  - ターミナル（CLI）上での可読性低下を防ぐため、**ユーザーへのチャット返答本文には生の Mermaid コードブロック（` ```mermaid ... ``` `）を直接出力しません**。
  - チャット返答では、テキスト・箇条書き・表を用いて要点を簡潔に伝えます。
  - 複雑なアーキテクチャ図やフロー図は **Markdown ドキュメント（`note/*.md`、`README.md` 等）内に記述** し、チャット本文からはそのドキュメントへのリンク（`[note/xx.md](file://...)`）を提示します。
- **ドキュメント内 Mermaid 構文エラーの撲滅**:
  - Markdown ファイル内で Mermaid 図を作成・更新する際は、ノードラベル、エッジテキスト（`-->|"..."|`）、SequenceDiagram の participant 名・Note 本文に含まれる特殊文字（`{`, `}`, `(`, `)`, `#`, `/`, `.`, `-` 等）を **必ずダブルクォート (`"..."`) でエスケープ・クォート** します。
